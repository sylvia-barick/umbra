...
export function TradeDrawer({
  ...
  }, [row, side, wallet.address]);

  const priceScale = 10n ** BigInt(priceDecimals);
  const quotePerUnit = row ? (side === "call" ? row.callQuote : row.putQuote) : null;
  ...
  async function submit() {
    if (!row || !wallet.address || estimatedPremium === null || bound === null) return;
    const sideVal: Side = side === "call" ? CALL : PUT;
    if (mode === "buy") {
      await run(
        async () => {
  ...
  return (
    <Modal open={!!row} onClose={onClose} title={`${underlyingSymbol} $${formatFixed(row.info.strike, tokenDecimals, 2)} · Series #${row.id}`}>
      <div className="space-y-5">
        ...
        <div>
          <label className="mb-1.5 block text-xs font-medium text-umbra-muted">
            Size ({underlyingSymbol} units)
          </label>
          <Input
            type="number"
            min="0"
        ...
        {!wallet.connected ? (
          ...
          <Button
            ...
            loading={busy}
            disabled={
              expired || sizeRaw <= 0n || estimatedPremium === null || quotePerUnit === null || insufficientForSell
            }
          >
            {insufficientForSell
              ? "Exceeds open position"
